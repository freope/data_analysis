...
class StrategyCombinatorWithOr(StrategyContinuousIndicatorAbstract):

    def __init__(self, strategies):
        """
        複数の戦略を OR 結合で連結させた戦略を生成する。
    ...
    def calculate_open_indicators(self, df):
        ...
        can_open_long = pd.concat(can_open_long, axis=1).any(axis=1)
        can_open_short = pd.concat(can_open_short, axis=1).any(axis=1)
