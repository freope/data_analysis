...
from frestu.agent.trader.strategy.part.stop_loss.strategy_part_stop_loss.calculate_with_cython import calculate_with_cython


class StrategyPartStopLoss(StrategyAbstract):
    
    def __init__(self, stopping_rate):
        """
        Parameters
        ----------
        stopping_rate : float
            損切りするレート差。レートそのものではないことに注意。
        """
        super().__init__()
    ...
    def __calculate_with_cython(self, df, opening_times, position_type):
        ...
        return calculate_with_cython(
            ...
            self.__stopping_rate, position_type)
    ...
    def __create_specifying(self, df, position_type):
        ...
        def _specify(opening_time):
            ...
            if position_type == 'long':
                stop_loss_rate = opening_rate - self.__stopping_rate
