...
from frestu.agent.trader.strategy.part.take_profit.strategy_part_take_profit.calculate_with_cython import calculate_with_cython


class StrategyPartTakeProfit(StrategyAbstract):
    
    def __init__(self, taking_rate):
        """
        Parameters
        ----------
        taking_rate : float
            利確するレート差。レートそのものではないことに注意。
        """
        super().__init__()
    ...
    def __calculate_with_cython(self, df, opening_times, position_type):
        ...
        return calculate_with_cython(
            ...
            self.__taking_rate, position_type)
    ...
    def __create_specifying(self, df, position_type):
        ...
        def _specify(opening_time):
            ...
            if position_type == 'long':
                take_profit_rate = opening_rate + self.__taking_rate
