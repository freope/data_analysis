...
from frestu.agent.trader.strategy.part.take_profit.strategy_part_take_profit_ratio.calculate_with_cython import calculate_with_cython


class StrategyPartTakeProfitRatio(StrategyAbstract):
    
    def __init__(self, taking_ratio):
        super().__init__()
        self.__taking_ratio = taking_ratio
    ...
    def __calculate_with_cython(self, df, opening_times, position_type):
        ...
        return calculate_with_cython(
            ...
            self.__taking_ratio, position_type)
    ...
    def __create_specifying(self, df, position_type):
        ...
        def _specify(opening_time):
            ...
            if position_type == 'long':
                take_profit_rate = opening_rate * (1 + self.__taking_ratio)
                indicators = df[opening_time:] >= take_profit_rate
