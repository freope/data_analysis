...
from frestu.agent.trader.strategy.trend import StrategyBbMasMstdMss


class StrategyHaBbMasMstdMss(StrategyConstructorAbstract):

    def __init__(self, window_ha, threshold, ma_type, window_bb):
        strategy_ha = StrategyHeikinashi(window_ha)
        strategy_bb = StrategyBbMasMstdMss(threshold, ma_type, window_bb)
